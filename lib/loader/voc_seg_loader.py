...
class VOCSegLoader(mx.io.DataIter):
    def __init__(self, image_root, label_root, data_list, batch_size, target_size,
            pad=False, shuffle=False, rand_scale=False, rand_mirror=False, rand_crop=False, data_slice=None, downsample=None):
...
def load_batch_semantic(image_src_list, label_src_list, target_size, scale_sampler, rand_scale, rand_mirror, rand_crop, downsample):
    img_batch, seg_batch = [], []
    for image_src, label_src in zip(image_src_list, label_src_list):
        img, seg = load_semantic(image_src, label_src, target_size, scale_sampler() if rand_scale else 1,
                                 rand_mirror and (np.random.rand() > 0.5), rand_crop, downsample)
