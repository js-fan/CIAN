...
def ResStemV1(data, num_filter, momentum=0.9, eps=1e-5, use_global_stats=False, bn_data=True,
...
def ResUnitV1(data, num_filter, stride, dilate, projection, bottle_neck, momentum=0.9, eps=1e-5,
        use_global_stats=False, name=None, lr_mult=1, reuse=None):
    assert name is not None

    if projection:
        shortcut = Conv(data, num_filter=num_filter, kernel=(1, 1), stride=(stride,)*2,
                        pad=(0, 0), no_bias=True, name=name+'_conv0', lr_mult=lr_mult, reuse=reuse)
        shortcut = BN(shortcut, fix_gamma=False, momentum=momentum, eps=eps, use_global_stats=use_global_stats,
                      name=name+'_bn0', lr_mult=lr_mult, reuse=reuse)
    else:
        shortcut = data

    if bottle_neck:
        x = Conv(data, num_filter=int(num_filter/4.), kernel=(1, 1), stride=(stride,)*2, pad=(0, 0),
        ...
        x = Conv(x, num_filter=int(num_filter/4.), kernel=(3, 3), stride=(1, 1), pad=(dilate,)*2,
...
def ResBlockV1(data, num_unit, num_filter, stride, dilate, bottle_neck, momentum=0.9, eps=1e-5,
             use_global_stats=False, name=None, lr_mult=1, reuse=None):
    assert name is not None
    x = ResUnitV1(data, num_filter, stride, dilate, True, bottle_neck, momentum, eps,
                  use_global_stats, name+'_unit1', lr_mult, reuse)
    for i in range(1, num_unit):
        x = ResUnitV1(x, num_filter, 1, dilate, False, bottle_neck, momentum, eps,
...
def _Resnet(x, num_units, num_filters, bottle_neck, momentum=0.9, eps=1e-5, use_global_stats=False, bn_data=True,
    ...
    for i in range(4):
        x = ResBlockV1(x, num_units[i], num_filters[i+1], strides[i], dilates[i], bottle_neck, 
